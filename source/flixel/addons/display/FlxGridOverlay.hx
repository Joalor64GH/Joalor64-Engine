...
class FlxGridOverlay
{
	...
	public static var gridMap:Map<String, BitmapData> = new Map();
	...
	{
		var mapName = '$CellWidth:$CellHeight:$Width:$Height:$Alternate:$Color1$Color2';
		if (gridMap.exists(mapName)) {
			return gridMap.get(mapName);
		}
		...
		gridMap.set(mapName, grid);
		return grid;
	}

	public static function clearCache() {
		for (key in gridMap.keys()) {
			var obj = gridMap.get(key);
			@:privateAccess
			if (obj != null) {
