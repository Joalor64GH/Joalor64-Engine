...
/**
 * Edit of FlxUINumericStepper to only update the value after you click off from the text field
 */
...
class FlxUINumericStepper extends FlxUIGroup implements IFlxUIWidget implements IFlxUIClickable implements IHasParams
{
	...
	var lastText = '';
	function _onInputTextEvent():Void
	{
        var text = text_field.text;
		if (text == lastText) return; //dont continue if nothing actually changed
		if (text == "")
		{
			text = Std.string(min);
		}
