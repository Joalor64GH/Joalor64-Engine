...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		#if HAXE_MODCHART
		var filesPushed:Array<String> = [];
		var foldersToCheck:Array<String> = [Paths.getPreloadPath('modcharts/')];

		#if MODS_ALLOWED
		foldersToCheck.insert(0, Paths.mods('modcharts/'));
		if(Paths.currentModDirectory != null && Paths.currentModDirectory.length > 0)
			foldersToCheck.insert(0, Paths.mods(Paths.currentModDirectory + '/modcharts/'));
		#end
		...
		timeBar.createFilledBar(0xFF000000, 0xFF00FFC8);
		...
		var versionTxt:FlxText = new FlxText(4, FlxG.height - 24, 0, '${SONG.song} ${CoolUtil.difficultyString()} - Joalor64 Engine v${MainMenuState.joalor64EngineVersion}', 12);
		...
		for (folder in foldersToCheck)
		{
			if(FileSystem.exists(folder))
			{
				for (file in FileSystem.readDirectory(folder))
				{
					if(file == '.hx' && !filesPushed.contains(file))
					{
						var expr = file;
						var parser = new hscript.Parser();
						var ast = parser.parseString(expr);
